...
# ── sidebar ──────────────────────────────────────────────────────────────────
with st.sidebar:
    st.image("https://img.icons8.com/color/96/box.png", width=60)
    st.title("設定")

    data_dir = st.text_input("資料夾路徑 (放所有 Excel)", str(DEFAULT_DATA_DIR))
    ...
    alert_days = st.slider("警示天數閾值 (實際預估 vs 需求 ≤ N 天)", 7, 90, 30)
...
if df_raw.empty:
    st.warning(f"⚠️ 在 `{data_dir}` 找不到任何可解析的 Excel 檔案，請確認路徑並放入報告後重新載入。")
    st.stop()
...
df["days_ros_gap"]     = (df["effective_delivery"] - df["ros"]).dt.days   # negative = late
df["is_arrived"]       = df["ata"].notna()
df["is_this_week"]     = (
    (df["effective_delivery"] >= week_start) &
    (df["effective_delivery"] <= week_end)
)
df["is_next_week"]     = (
    (df["effective_delivery"] > week_end) &
    (df["effective_delivery"] <= next_week_end)
)
...
def alert_level(row):
    ...
    if gap <= 0:
        return "critical"   # already past ROS or will be late
    if gap <= alert_days:
        return "warning"
    return "ok"
...
total      = len(df_raw)
arrived    = df_raw["ata"].notna().sum()
critical_n = (df["alert"] == "critical").sum()
warning_n  = (df["alert"] == "warning").sum()
this_wk_n  = df["is_this_week"].sum()
next_wk_n  = df["is_next_week"].sum()

col1.metric("📋 總材料項目", total)
col2.metric("✅ 已到料", int(arrived))
col3.metric("🔴 緊急警示", int(critical_n))
col4.metric("🟠 一個月內到期", int(warning_n))
col5.metric("📅 本週 / 次週預計到料", f"{int(this_wk_n)} / {int(next_wk_n)}")
...
with tab2:
    ...
    if alert_df.empty:
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "差距(天)": st.column_config.NumberColumn(format="%d 天"),
            }
        )
        st.download_button(
            "⬇️ 下載警示清單 CSV",
...
with tab3:
    ...
    with c2:
        weeks_ahead = st.slider("顯示未來幾週", 2, 24, 8)

    if selected_po != "全部":
        gantt_df = gantt_df[gantt_df["po_no"] == selected_po]

    cutoff = today + timedelta(weeks=weeks_ahead)
    gantt_df = gantt_df[gantt_df["effective_delivery"] <= cutoff]
    ...
    if gantt_df.empty:
        ...
        gantt_df["Color"] = gantt_df["alert"].map({
            "critical": "緊急", "warning": "注意", "ok": "正常", "arrived": "已到料", "unknown": "未知"
        })
        gantt_df["label"] = gantt_df.apply(
            lambda r: f"{r['item_name']} ({r['po_no']})", axis=1
        )
    ...
    # Timeline scatter: days_to_delivery
    st.subheader("📈 到料時程散佈圖 (距今天數 vs ROS 差距)")
